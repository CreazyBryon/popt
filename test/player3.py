...
def play_one_game(player_cards, game_number):
	...
	# If rock is 0, only play rounds 1-2; otherwise play all 7
	max_round = 2 if player_cards["rock"] == 0 else 7

	print(f"Game {game_number}")
	print(f"Cards before game: {rps.format_card_counts(player_cards)}")
	print(f"Strategy: {'Rounds 1-2 only (no rock)' if max_round == 2 else 'Full 7 rounds'}")

	for round_number in range(1, max_round + 1):
		while True:
			if rps.total_cards_left(player_cards) == 0:
				print("Player 3 cannot throw because all cards are 0.")
			...
			print(f"Player 3 threw: {player_choice}")
			print(f"Round result: {result}")

			if any_card_empty(player_cards):
				print("Player 3 stops because one card type reached 0.")
			...
			if result == "draw":
				print("Player 3 retries the same round.\n")
				continue

			if result == "lose":
				print("Player 3 failed the run and gets 0 gold coin(s).")
				print(f"Cards left: {rps.format_card_counts(player_cards)}")
				print()
				return 0, False
	...
	print(f"Player 3 stops after round {max_round}.")
	print(f"Coins taken: {coins}")
	print(f"Cards left: {rps.format_card_counts(player_cards)}")
	print(f"Result summary: {round_results}")
	print()
...
def play_until_any_card_empty():
	player_cards = STARTING_CARDS.copy()
	total_coins = 0
	game_number = 1

	if rps.total_cards_left(player_cards) == 0:
		print("Player 3 cannot start because total cards are 0.")
		return
	...
	if any_card_empty(player_cards):
		print("Player 3 ended because at least one card type reached 0.")
	else:
