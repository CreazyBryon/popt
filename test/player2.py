...
def play_one_game(player_cards, game_number):
	...
	for round_number in range(1, 6):
		...
		if player_choice is None:
			print("Player 2 cannot throw because all cards are 0.")
		...
		print(f"Player 2 threw: {player_choice}")
		print(f"Round result: {result}")

		if any_card_empty(player_cards):
			print("Player 2 stops because one card type reached 0.")
		...
		if result == "draw":
			print("Player 2 stops immediately because of a draw.")
			print(f"Coins taken: {coins}")
			print(f"Cards left: {rps.format_card_counts(player_cards)}")
			print(f"Result summary: {round_results}")
			print()
		...
		if result == "lose":
			print("Player 2 failed the run and gets 0 gold coin(s).")
			print(f"Cards left: {rps.format_card_counts(player_cards)}")
			print()
			return 0, False
	...
	print("Player 2 stops after round 5.")
	print(f"Coins taken: {coins}")
	print(f"Cards left: {rps.format_card_counts(player_cards)}")
	print(f"Result summary: {round_results}")
	print()
...
def play_until_all_cards_consumed():
	...
	if rps.total_cards_left(player_cards) == 0:
		print("Player 2 cannot start because total cards are 0.")
		return
	...
	if any_card_empty(player_cards):
		print("Player 2 ended because at least one card type reached 0.")
	else:
